...
const DriverDashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-zinc-50 text-black font-sans flex flex-col">
      {/* Header */}
      <header className="border-b border-zinc-200 bg-white/70 backdrop-blur-md px-6 py-4 flex justify-between items-center sticky top-0 z-50">
        <div>
      ...
      <main className="flex-1 max-w-lg mx-auto w-full p-4 space-y-4">
        ...
        {gpsError && (
          <div className="bg-zinc-100 border border-zinc-300 text-xs text-black p-4 rounded-xl text-center font-semibold">
            ⚠️ GPS: {gpsError}
          </div>
        )}

        {/* Bus Information Panel */}
        <div className="glass-panel p-6 border-zinc-200">
          {assignedBus ? (
            <div className="space-y-4">
              <div>
                <span className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Assigned Vehicle</span>
                <div className="flex items-baseline justify-between mt-1">
                  <h2 className="text-3xl font-extrabold text-black">{assignedBus.busCode}</h2>
                  <span className="text-sm font-mono text-zinc-650 bg-zinc-100/50 border border-zinc-200 px-2 py-0.5 rounded">
              ...
              {assignedBus.assignedRoute ? (
                <div>
                  ...
                  <p className="text-sm font-semibold text-black mt-0.5">{assignedBus.assignedRoute.name}</p>
        ...
        {assignedBus && (
          <div className="glass-panel p-6 border-zinc-200 flex flex-col items-center justify-center space-y-6">
            {!activeTrip ? (
              <button
              ...
              <div className="w-full space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4 text-center">
                  <div className="bg-zinc-100/40 border border-zinc-200 rounded-xl p-3">
                  ...
                  <div className="bg-zinc-100/40 border border-zinc-200 rounded-xl p-3 flex flex-col justify-center">
                    ...
                    <span className="text-sm font-bold flex items-center justify-center gap-1.5 mt-0.5">
                      <span className={`w-2.5 h-2.5 rounded-full ${socketConnected && isTracking ? 'bg-black' : 'bg-zinc-300 animate-pulse'}`} />
                ...
                <button
                  ...
                  disabled={actionLoading}
                  className="w-full h-40 bg-black text-white font-bold border border-black hover:bg-white hover:text-black hover:border-black transition-all rounded-3xl flex flex-col items-center justify-center active:scale-95 duration-200"
                >
                  <span className="text-4xl">🏁</span>
        ...
        {activeTrip && (
          <div className="glass-panel p-4 border-zinc-200 space-y-2">
            <span className="text-xs font-semibold uppercase tracking-wider text-zinc-500">GPS Transmission Log</span>
            <div className="bg-zinc-100/60 border border-zinc-200 rounded-xl p-3 h-32 overflow-y-auto font-mono text-[10px] text-zinc-650 space-y-1 custom-scrollbar">
              {logs.length === 0 ? (
                <p className="text-zinc-500 italic">Waiting for initial coordinate updates...</p>
              ) : (
        ...
        <div className="bg-zinc-100/40 border border-zinc-200 rounded-xl p-4 text-[11px] text-zinc-500 space-y-2">
